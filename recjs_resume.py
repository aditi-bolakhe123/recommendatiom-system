...
def parse_recommendation_text(recommendation_text):
    # Initialize dictionary to store parsed recommendations
    parsed_recommendations = {}

    # Use regex to extract sections based on numbered list and headings
    sections = re.split(r'\n\s*(\d+\.\s*.+)\s*(?=^\d+\.\s*|\Z)', recommendation_text.strip(), flags=re.MULTILINE)

    # Iterate through sections
    for section in sections:
        if section.strip():  # Ensure section is not empty
            # Split section into heading and recommendations
            heading, *recommendations = section.strip().split('\n')
            heading = heading.strip(':')

            # Strip unnecessary characters and add to parsed_recommendations
...
with st.form("Student_details_form"):
    ...
    if submitted:
        ...
        resume_text = ""
        if uploaded_resume:
            resume_text = extract_resume_info(uploaded_resume)
        ...
        # Parse the recommendation text into a structured dictionary
